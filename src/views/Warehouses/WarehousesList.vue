    ...
    <div class="space-y-5 sm:space-y-6">
      <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Omborlar</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Omborxonalarni boshqarish</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <ActionIconButton action="create" :title="t('actions.newWarehouse')" @click="openCreate" />
          </div>
        </div>
    ...
    <Modal v-if="showFormModal" @close="closeForm">
      <template #body>
        <div class="relative w-full max-w-md p-6 bg-white rounded-3xl dark:bg-gray-900">
          <h4 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">{{ editing ? 'Omborni tahrirlash' : 'Yangi ombor' }}</h4>
          ...
          <form @submit.prevent="submitForm">
            ...
            <input v-model="formName" type="text" required :class="inputClass" placeholder="Masalan: Markaziy ombor" />
...
import { fetchAllWarehouses, createWarehouse, updateWarehouse, deleteWarehouse, type WarehouseResponse } from '@/services/warehouses'
...
const currentPageTitle = ref('Omborlar')
const items = ref<WarehouseResponse[]>([])
const loading = ref(false)
const errorMessage = ref('')
const search = ref('')
const { visible, toggleColumn, isVisible } = useColumnVisibility(TABLE_COLUMNS, 'warehouses-cols')
const { sortKey, sortDir, toggleSort, applySort } = useTableSort<WarehouseResponse>((row, key) => row[key as keyof WarehouseResponse])
...
const itemToDelete = ref<WarehouseResponse | null>(null)
...
const loadItems = async () => {
  ...
  try { items.value = await fetchAllWarehouses() } catch (e) { errorMessage.value = e instanceof Error ? e.message : 'Yuklashda xatolik' } finally { loading.value = false }
}
const openCreate = () => { editing.value = null; formName.value = ''; formError.value = ''; showFormModal.value = true }
const openEdit = (item: WarehouseResponse) => { editing.value = item; formName.value = item.name; formError.value = ''; showFormModal.value = true }
const closeForm = () => { showFormModal.value = false }
const submitForm = async () => {
  formError.value = ''; saving.value = true
  try {
    const name = formName.value.trim()
    if (editing.value) await updateWarehouse(editing.value.id, { name })
    else await createWarehouse({ name })
...
const confirmDelete = (item: WarehouseResponse) => { itemToDelete.value = item; showDeleteModal.value = true }
const doDelete = async () => {
  if (!itemToDelete.value) return; deleting.value = true
  try { await deleteWarehouse(itemToDelete.value.id); showDeleteModal.value = false; await loadItems() }
