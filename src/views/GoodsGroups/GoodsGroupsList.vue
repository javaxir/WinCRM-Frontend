    ...
    <div class="space-y-5 sm:space-y-6">
      <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Mahsulot guruhlari</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Mahsulotlarni guruhlash (masalan: Ichimliklar, Profil)</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <ActionIconButton action="create" :title="t('actions.newGroup')" @click="openCreate" />
          </div>
        </div>
    ...
    <Modal v-if="showFormModal" @close="closeForm">
      <template #body>
        <div class="relative w-full max-w-md p-6 bg-white rounded-3xl dark:bg-gray-900">
          <h4 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">{{ editing ? 'Guruhni tahrirlash' : 'Yangi guruh' }}</h4>
          ...
          <form @submit.prevent="submitForm">
            ...
            <input v-model="formName" type="text" required :class="inputClass" placeholder="Masalan: Ichimliklar" />
...
import { fetchAllGoodsGroups, createGoodsGroup, updateGoodsGroup, deleteGoodsGroup, type GoodsGroupResponse } from '@/services/goodsGroups'
...
const currentPageTitle = ref('Mahsulot guruhlari')
const items = ref<GoodsGroupResponse[]>([])
const loading = ref(false)
const errorMessage = ref('')
const search = ref('')
const { visible, toggleColumn, isVisible } = useColumnVisibility(TABLE_COLUMNS, 'goods-groups-cols')
const { sortKey, sortDir, toggleSort, applySort } = useTableSort<GoodsGroupResponse>((row, key) => row[key as keyof GoodsGroupResponse])
...
const editing = ref<GoodsGroupResponse | null>(null)
const formName = ref('')
const formError = ref('')
const saving = ref(false)
const showDeleteModal = ref(false)
const itemToDelete = ref<GoodsGroupResponse | null>(null)
...
const loadItems = async () => {
  ...
  try { items.value = await fetchAllGoodsGroups() } catch (e) { errorMessage.value = e instanceof Error ? e.message : 'Yuklashda xatolik' } finally { loading.value = false }
}
const openCreate = () => { editing.value = null; formName.value = ''; formError.value = ''; showFormModal.value = true }
const openEdit = (item: GoodsGroupResponse) => { editing.value = item; formName.value = item.name; formError.value = ''; showFormModal.value = true }
const closeForm = () => { showFormModal.value = false }
const submitForm = async () => {
  formError.value = ''; saving.value = true
  try {
    const name = formName.value.trim()
    if (editing.value) await updateGoodsGroup(editing.value.id, { name })
    else await createGoodsGroup({ name })
    showFormModal.value = false; await loadItems()
  } catch (e) { formError.value = e instanceof Error ? e.message : 'Saqlashda xatolik' } finally { saving.value = false }
}
const confirmDelete = (item: GoodsGroupResponse) => { itemToDelete.value = item; showDeleteModal.value = true }
const doDelete = async () => {
  if (!itemToDelete.value) return; deleting.value = true
  try { await deleteGoodsGroup(itemToDelete.value.id); showDeleteModal.value = false; await loadItems() }
  catch (e) { errorMessage.value = e instanceof Error ? e.message : 'O‘chirishda xatolik'; showDeleteModal.value = false } finally { deleting.value = false }
}
onMounted(loadItems)
</script>
