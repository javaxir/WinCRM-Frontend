<template>
  <AdminLayout>
    ...
    <div class="space-y-5 sm:space-y-6">
      <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">{{ t('clientGroups.title') }}</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ t('clientGroups.subtitle') }}</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <ActionIconButton action="create" :title="t('actions.newClientGroup')" @click="openCreate" />
          </div>
        </div>
        ...
        <div class="px-6 pb-4 pt-4 border-t border-gray-100 dark:border-gray-800">
          <input v-model="search" type="text" :placeholder="t('common.search')" :class="inputClass + ' max-w-sm'" />
        </div>
            ...
            <table class="min-w-full">
              <thead>
                <tr class="border-b border-gray-200 dark:border-gray-700">
                  ...
                  <SortableTh v-if="isVisible('name')" :label="t('clientGroups.name')" sortable :active="sortKey === 'name'" :direction="sortKey === 'name' ? sortDir : null" @sort="toggleSort('name')" />
                  ...
                  <SortableTh v-if="isVisible('createdUsername')" :label="t('clientGroups.createdBy')" sortable :active="sortKey === 'createdUsername'" :direction="sortKey === 'createdUsername' ? sortDir : null" @sort="toggleSort('createdUsername')" />
                  <SortableTh v-if="isVisible('createdAt')" :label="t('clientGroups.createdAt')" sortable :active="sortKey === 'createdAt'" :direction="sortKey === 'createdAt' ? sortDir : null" @sort="toggleSort('createdAt')" />
                  <SortableTh :label="t('common.actions')" align="right" />
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-if="loading"><td :colspan="colCount + 1" class="px-5 py-8 text-center text-gray-500">{{ t('common.loading') }}</td></tr>
                <tr v-else-if="displayItems.length === 0"><td :colspan="colCount + 1" class="px-5 py-8 text-center text-gray-500">{{ t('common.notFound') }}</td></tr>
    ...
    <Modal v-if="showFormModal" @close="closeForm">
      <template #body>
        <div class="relative w-full max-w-md p-6 bg-white rounded-3xl dark:bg-gray-900">
          <h4 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">{{ editing ? t('clientGroups.editTitle') : t('clientGroups.createTitle') }}</h4>
          ...
          <form @submit.prevent="submitForm">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ t('clientGroups.name') }}<span class="text-error-500">*</span></label>
            <input v-model="formName" type="text" required :class="inputClass" :placeholder="t('clientGroups.namePlaceholder')" />
    ...
    <Modal v-if="showDeleteModal" @close="showDeleteModal = false">
      <template #body>
        <div class="relative w-full max-w-md p-6 bg-white rounded-3xl dark:bg-gray-900">
          <h4 class="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">{{ t('common.delete') }}</h4>
          <p class="mb-6 text-sm text-gray-500"><span class="font-medium text-gray-700 dark:text-gray-300">{{ itemToDelete?.name }}</span> {{ t('clientGroups.deleteConfirm') }}</p>
          <div class="flex justify-end gap-3">
            <button @click="showDeleteModal = false" :class="btnOutline">{{ t('common.cancel') }}</button>
            <button @click="doDelete" :disabled="deleting" class="rounded-lg bg-red-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-red-700 disabled:opacity-70">{{ deleting ? t('common.deleting') : t('common.delete') }}</button>
          </div>
        </div>
      </template>
    </Modal>
...
import {
  fetchAllClientGroups,
  createClientGroup,
  updateClientGroup,
  deleteClientGroup,
  type ClientGroupResponse,
} from '@/services/clientGroups'
...
const items = ref<ClientGroupResponse[]>([])
const loading = ref(false)
const errorMessage = ref('')
const search = ref('')
const { visible, toggleColumn, isVisible } = useColumnVisibility(TABLE_COLUMNS, 'client-groups-cols')
const { sortKey, sortDir, toggleSort, applySort } = useTableSort<ClientGroupResponse>((row, key) => row[key as keyof ClientGroupResponse])
...
const editing = ref<ClientGroupResponse | null>(null)
const formName = ref('')
const formError = ref('')
const saving = ref(false)
const showDeleteModal = ref(false)
const itemToDelete = ref<ClientGroupResponse | null>(null)
...
const loadItems = async () => {
  ...
  errorMessage.value = ''
  try {
    items.value = await fetchAllClientGroups()
  } catch (e) {
...
const openEdit = (item: ClientGroupResponse) => {
  editing.value = item
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editing.value) await updateClientGroup(editing.value.id, { name })
    else await createClientGroup({ name })
    showFormModal.value = false
    await loadItems()
  } catch (e) {
...
const confirmDelete = (item: ClientGroupResponse) => {
...
const doDelete = async () => {
  ...
  deleting.value = true
  try {
    await deleteClientGroup(itemToDelete.value.id)
...
onMounted(() => {
  currentPageTitle.value = t('clientGroups.title')
  loadItems()
})
</script>
