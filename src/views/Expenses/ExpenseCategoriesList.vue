    ...
    <div class="space-y-5 sm:space-y-6">
      <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-5">
          <div>
            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Xarajat kategoriyalari</h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Xarajatlar uchun kategoriyalarni boshqarish</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <ActionIconButton action="create" :title="t('actions.newExpenseCategory')" @click="openCreate" />
          </div>
        </div>
    ...
    <Modal v-if="showFormModal" @close="closeForm">
      <template #body>
        <div class="relative w-full max-w-md p-6 bg-white rounded-3xl dark:bg-gray-900">
          <h4 class="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">{{ editing ? 'Tahrirlash' : 'Yangi kategoriya' }}</h4>
...
const currentPageTitle = ref('Xarajat kategoriyalari')
const items = ref<ExpenseCategoryResponse[]>([])
const loading = ref(false)
const errorMessage = ref('')
const search = ref('')

const { visible, toggleColumn, isVisible } = useColumnVisibility(TABLE_COLUMNS, 'expense-categories-cols')
const { sortKey, sortDir, toggleSort, applySort } = useTableSort<ExpenseCategoryResponse>((row, key) => row[key as keyof ExpenseCategoryResponse])
...
const editing = ref<ExpenseCategoryResponse | null>(null)
const form = ref({ name: '', description: '' })
const formError = ref('')
const saving = ref(false)
const showDeleteModal = ref(false)
const itemToDelete = ref<ExpenseCategoryResponse | null>(null)
...
const loadItems = async () => {
  ...
  errorMessage.value = ''
  try {
    items.value = await fetchAllExpenseCategories()
  } catch (e) {
...
const openEdit = (item: ExpenseCategoryResponse) => {
  editing.value = item
...
const submitForm = async () => {
  ...
  try {
    ...
    if (editing.value) await updateExpenseCategory(editing.value.id, payload)
...
const confirmDelete = (item: ExpenseCategoryResponse) => { itemToDelete.value = item; showDeleteModal.value = true }
